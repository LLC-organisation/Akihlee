...
// Below this width the slide-over doesn't have room for a message list and
// a history panel both being useful at once, so the launcher sends straight
// to the full /ai-cfo page instead — one modern chat UI, not a cramped
// second one. Matches AppSidebar's lg breakpoint for mobile vs. desktop.
 ...
 * the full /ai-cfo page. Shares the same persisted-conversation backend
 * as /ai-cfo (not a separate stateless chat) so a conversation started
 * here shows up in that page's history and vice versa — "Expand to full
 * screen" hands off the same conversation rather than starting over.
 */
export function AiCfoChatWidget() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      // Non-fatal — history just stays empty/retryable, chat itself still works.
    } finally {
      setLoadingConversations(false);
    }
  }, []);

  // Launcher click: on mobile, skip the slide-over entirely and go straight
  // to the full page — see MOBILE_BREAKPOINT_QUERY above.
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                {view === 'history' ? (
                  <button
                    ...
                    title="Back to chat"
                    className="p-1.5 -ml-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors duration-200 shrink-0"
                  >
              ...
              <div className="flex items-center gap-1 shrink-0">
                {view === 'chat' && (
                  <>
                    ...
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2M21 12a9 9 0 11-9-9 9 9 0 019 9z" />
                      </svg>
                    </button>
                    <button
                      type="button"
